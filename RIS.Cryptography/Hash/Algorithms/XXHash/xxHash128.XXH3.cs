...
namespace RIS.Cryptography.Hash.Algorithms
{
    ...
    public static partial class XXHash128
    {
        ...
        private static unsafe UInt128 XXH3_128bits_internal(
            byte* input, int len, ulong seed, byte* secret, int secretLen)
        {
            if (len <= 16)
            ...
            return XXH3_hashLong_128b_withSeed(input, len, seed, secret, secretLen);
        }



        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe UInt128 XXH3_len_1to3_128b(
            byte* input, int len, byte* secret, ulong seed)
        {
            var c1 = input[0];
            var c2 = input[len >> 1];
            var c3 = input[len - 1];

            var combinedl = ((uint)c1 << 16) |
                            ((uint)c2 << 24) |
                            ((uint)c3 << 0) |
                            ((uint)len << 8);
            var combinedh = XXH_rotl32(XXH_swap32(combinedl), 13);

            var bitflipl = (XXH_readLE32(secret) ^ XXH_readLE32(secret + 4)) + seed;
            var bitfliph = (XXH_readLE32(secret + 8) ^ XXH_readLE32(secret + 12)) - seed;
            var keyed_lo = (ulong)combinedl ^ bitflipl;
            var keyed_hi = (ulong)combinedh ^ bitfliph;

            return new UInt128(XXH64_avalanche(keyed_hi), XXH64_avalanche(keyed_lo));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe UInt128 XXH3_len_4to8_128b(
            byte* input, int len, byte* secret, ulong seed)
        {
            seed ^= (ulong)XXH_swap32((uint)seed) << 32;

            var input_lo = XXH_readLE32(input);
            var input_hi = XXH_readLE32(input + len - 4);
            var input_64 = input_lo + ((ulong)input_hi << 32);
            var bitflip = (XXH_readLE64(secret + 16) ^ XXH_readLE64(secret + 24)) + seed;
            ...
            lower = XXH_xorshift64(lower, 35);
            lower *= 0x9FB21C651E98DF25UL;
            lower = XXH_xorshift64(lower, 28);
            upper = XXH3_avalanche(upper);

            return new UInt128(upper, lower);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe UInt128 XXH3_len_0to16_128b(
            byte* input, int len, byte* secret, ulong seed)
        {
            if (len > 8)
            ...
            var bitflipl = XXH_readLE64(secret + 64) ^ XXH_readLE64(secret + 72);
            var bitfliph = XXH_readLE64(secret + 80) ^ XXH_readLE64(secret + 88);

            return new UInt128(XXH64_avalanche(seed ^ bitfliph), XXH64_avalanche(seed ^ bitflipl));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe UInt128 XXH3_len_9to16_128b(
            byte* input, int len, byte* secret, ulong seed)
        {
            var bitflipl = (XXH_readLE64(secret + 32) ^ XXH_readLE64(secret + 40)) - seed;
            var bitfliph = (XXH_readLE64(secret + 48) ^ XXH_readLE64(secret + 56)) + seed;
            var input_lo = XXH_readLE64(input);
            var input_hi = XXH_readLE64(input + len - 8);
            ...
            upper += input_hi + XXH_mult32to64((uint)input_hi, XXH_PRIME32_2 - 1);
            lower ^= XXH_swap64(upper);
            ...
            lowerH = XXH3_avalanche(lowerH);
            upperH = XXH3_avalanche(upperH);

            return new UInt128(upperH, lowerH);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe UInt128 XXH3_len_17to128_128b(
            byte* input, int len, byte* secret, int secretSize, ulong seed)
        {
            var acc = new UInt128(0, (ulong) len * XXH_PRIME64_1);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe UInt128 XXH3_len_129to240_128b(
            byte* input, int len, byte* secret, int secretSize, ulong seed)
        {
            var nbRounds = len / 32;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static ulong XXH3_avalanche(
            ulong h64)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe ulong XXH3_mix16B(
            byte* input, byte* secret, ulong seed)
        {
            var input_lo = XXH_readLE64(input);
            var input_hi = XXH_readLE64(input + 8);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static ulong XXH3_mul128_fold64(
            ulong lhs, ulong rhs)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe UInt128 XXH3_hashLong_128b_withSeed(
            byte* input, int len, ulong seed, byte* secret, int secretSize)
        {
            if (seed == 0)
                return XXH3_hashLong_128b_internal(input, len, secret, secretSize);

            var customSecret = stackalloc byte[XXH3_SECRET_DEFAULT_SIZE];
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe UInt128 XXH3_hashLong_128b_internal(
            byte* input, int len, byte* secret, int secretSize)
        {
            fixed (ulong* src = &XXH3_INIT_ACC[0])
            {
                var acc = stackalloc ulong[8]
                {
                    *(src + 0),
                    *(src + 1),
                    *(src + 2),
                ...
                var lower = XXH3_mergeAccs(acc,
                    secret + XXH_SECRET_MERGEACCS_START,
                    (ulong)len * XXH_PRIME64_1);
                var upper = XXH3_mergeAccs(acc,
        ...
        {
            var nbStripesPerBlock = (secretSize - XXH_STRIPE_LEN) / XXH_SECRET_CONSUME_RATE;
            var block_len = XXH_STRIPE_LEN * nbStripesPerBlock;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe ulong XXH3_mergeAccs(
            ulong* acc, byte* secret, ulong start)
        {
            var result64 = start;
        ...
        private static unsafe void XXH3_accumulate(
            ulong* acc, byte* input, byte* secret, int nbStripes)
        {
            for (var n = 0; n < nbStripes; ++n)
            {
                var inp = input + n * XXH_STRIPE_LEN;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe void XXH3_scalarRound(
            ulong* acc, byte* input, byte* secret, int lane)
        {
            var xacc = acc;
            var xinput = input;
            var xsecret = secret;

            var data_val = XXH_readLE64(xinput + lane * 8);
            var data_key = data_val ^ XXH_readLE64(xsecret + lane * 8);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe void XXH3_scrambleAcc_sse2(
            ulong* acc, byte* secret)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe void XXH3_scalarScrambleRound(
            ulong* acc, byte* secret, int lane)
        {
            var xacc = acc;
            var xsecret = secret;

            var key64 = XXH_readLE64(xsecret + lane * 8);
            var acc64 = xacc[lane];
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe void XXH3_initCustomSecret_sse2(
            byte* customSecret, ulong seed64)
        {
            ...
            byte* customSecret, ulong seed)
        {
            ...
            {
                var nbRounds = XXH_SECRET_DEFAULT_SIZE / 16;

                for (var i = 0; i < nbRounds; ++i)
                {
                    var lo = XXH_readLE64(kSecretPtr + 16 * i) + seed;
                    var hi = XXH_readLE64(kSecretPtr + 16 * i + 8) - seed;
