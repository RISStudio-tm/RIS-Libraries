...
namespace RIS.Cryptography.Hash.Algorithms
{
    ...
    public static partial class XXHash3
    {
        ...
        private static unsafe ulong XXH3_64bits_internal(
            byte* input, int len, ulong seed64, byte* secret, int secretLen)
        {
            if (len <= 16)
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe ulong XXH3_len_1to3_64b(
            byte* input, int len, byte* secret, ulong seed)
        {
            ...
            var combined = ((uint)c1 << 16) |
                           ((uint)c2 << 24) |
                           ((uint)c3 << 0) |
                           ((uint)len << 8);

            var bitflip = (XXH_readLE32(secret) ^
                           XXH_readLE32(secret + 4)) + seed;
            var keyed = (ulong)combined ^ bitflip;

            return XXH64_avalanche(keyed);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe ulong XXH3_len_4to8_64b(
            byte* input, int len, byte* secret, ulong seed)
        {
            seed ^= (ulong)XXH_swap32((uint)seed) << 32;

            var input1 = XXH_readLE32(input);
            var input2 = XXH_readLE32(input + len - 4);
            var bitflip = (XXH_readLE64(secret + 8) ^ XXH_readLE64(secret + 16)) - seed;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe ulong XXH3_len_0to16_64b(
            byte* input, int len, byte* secret, ulong seed)
        {
            if (len > 8)
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe ulong XXH3_len_9to16_64b(
            byte* input, int len, byte* secret, ulong seed)
        {
            var bitflip1 = (XXH_readLE64(secret + 24) ^ XXH_readLE64(secret + 32)) + seed;
            var bitflip2 = (XXH_readLE64(secret + 40) ^ XXH_readLE64(secret + 48)) - seed;
            var input_lo = XXH_readLE64(input) ^ bitflip1;
            var input_hi = XXH_readLE64(input + len - 8) ^ bitflip2;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe ulong XXH3_len_17to128_64b(
            byte* input, int len, byte* secret, int secretSize, ulong seed)
        {
            var acc = (ulong)len * XXH_PRIME64_1;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe ulong XXH3_len_129to240_64b(
            byte* input, int len, byte* secret, int secretSize, ulong seed)
        {
            var nbRounds = len / 16;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static ulong XXH3_rrmxmx(
            ulong h64, int len)
        {
        ...
        private static unsafe ulong XXH3_mix16B(
            byte* input, byte* secret, ulong seed64)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe ulong XXH3_hashLong_64b_withSeed(
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe ulong XXH3_hashLong_64b_internal(
