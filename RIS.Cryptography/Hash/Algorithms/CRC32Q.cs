...
{
    public sealed class CRC32Q : HashAlgorithm
    {
        private const uint Polynomial = 0xD5828281;
        private static readonly uint[] Table = new uint[16 * 256];
        private uint CurrentInitial { get; set; }

        public new static CRC32Q Create()
        {
            return new CRC32Q();
        }
        public new static CRC32Q Create(string hashName)
        {
            return new CRC32Q();
        }
        ...
        private static uint AppendInternal(uint initial, byte[] input, int offset, int length)
        {
            ...
            return Environment.ReflectBits(crcLocal) ^ 0x0;
