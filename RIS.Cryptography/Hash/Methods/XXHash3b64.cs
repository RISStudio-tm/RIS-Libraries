...
{
    public sealed class XXHash3b64 : IRawHashMethod
    {
        private uint _seed;
        public uint Seed
        {
        ...
        public XXHash3b64(uint? seed = null)
        {
            if (seed == null)
            {
        ...
        {
            return BitConverter.GetBytes(Algorithms.XXHash3.ComputeHash(plainText, Seed));
        }
        public byte[] GetHashBytes(byte[] data)
        {
            return BitConverter.GetBytes(Algorithms.XXHash3.ComputeHash(data, data.Length, Seed));
        }
