...
namespace RIS.Cryptography.Hash.Methods
{
    public sealed class Argon2idWP : IHashMethod
    {
            ...
            int degreeOfParallelism, string associatedData, string knownSecret)
        {
            ...
            return $"${Enum.GetName(typeof(Argon2Type), Argon2Type.Argon2id)?.ToLower()}$v=19$m={memorySize},t={iterations},p={degreeOfParallelism}${Base64.RemovePadding(Convert.ToBase64String(hashSalt))}${Base64.RemovePadding(Convert.ToBase64String(SecureUtils.GetBytes(hashText.ToString())))}";
        }
