...
namespace RIS.Cryptography.Hash.Methods
{
    public sealed class XXHash64 : IRawHashMethod
    {
        ...
        {
            return BitConverter.GetBytes(Algorithms.XXHash64.ComputeHash(plainText, Seed));
        }
        public byte[] GetHashBytes(byte[] data)
        {
            return BitConverter.GetBytes(Algorithms.XXHash64.ComputeHash(data, data.Length, Seed));
        }
