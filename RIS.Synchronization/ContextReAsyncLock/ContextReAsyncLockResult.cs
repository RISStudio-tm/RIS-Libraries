...
{
    public sealed class ContextReAsyncLockResult<T> : IAwaiter<T>, IAwaitable<T, ContextReAsyncLockResult<T>>
    {
        ...
        public ContextReAsyncLockResult(
...
#pragma warning restore ParallelChecker
        ...
        public ContextReAsyncLockResult<T> GetAwaiter()
        {
            return this;
        }
            ...
            Action continuation)
        {
            if (Interlocked.Exchange(ref _hasCompleted, 1) != 0)
            {
                throw new InvalidOperationException(
                    $"This method must only be called once. Simply await {nameof(ContextReAsyncLockResult<T>)}.");
            }
