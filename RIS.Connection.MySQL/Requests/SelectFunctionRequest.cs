...
namespace RIS.Connection.MySQL.Requests
{
    ...
    /// </summary>
    public sealed class SelectFunctionRequest : IQueryRequest<string>
    {
        /// <summary>
        ...
        /// <summary>
        ///     Позволяет получать или устанавливать таблицу, для которой будет вызван SELECT. Передайте null или пустую строку, чтобы не использовать привязку к таблице.
        /// </summary>
        ...
        private SelectFunctionRequest(IRequestEngine engine, SelectFunctionRequest request)
            : this(engine, request.Name, request.ParametersValues, request.Table, request.Timeout, request.IsolationLevel)
        {

        }
        /// <summary>
        ///     Инициализирует новый экземпляр класса <see cref="RIS.Connection.MySQL.Requests.SelectFunctionRequest"/>.
        /// </summary>
        /// <param name="engine">
        ...
        /// </param>
        public SelectFunctionRequest(IRequestEngine engine,
            string name, string[] parametersValues, string table,
            IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
            : this(engine, name, parametersValues, table, engine.CommandTimeout, isolationLevel)
        {
        ...
        public async Task<string> ExecuteAsync()
        {
            ...
            Task<Task> request = Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    sqlBuilder.Append("SELECT ").Append(Name).Append('(');

                    if (ParametersValues.Length != 0)
                    {
                        sqlBuilder.Append("@param0");
                        command.Parameters.AddWithValue("@param0", ParametersValues[0]);

                        RequestEngineHelper.ReplaceDBNullParameterValue(ParametersValues[0],
                            ref command, command.Parameters[0].ParameterName);
                        RequestEngineHelper.ReplaceFunctionParameterValue(ParametersValues[0],
                        ...
                        for (int i = 1; i < ParametersValues.Length; ++i)
                        {
                            ...
                            command.Parameters.AddWithValue($"@param{i}", ParametersValues[i]);

                            RequestEngineHelper.ReplaceDBNullParameterValue(ParametersValues[i],
                                ref command, command.Parameters[i].ParameterName);
                            RequestEngineHelper.ReplaceFunctionParameterValue(ParametersValues[i],
        ...
        ///     Создаёт копию запроса.
        /// </summary>
        public SelectFunctionRequest Copy()
        {
            return Copy(Engine);
        }
        /// <summary>
        ...
        ///     Экземпляр сервиса <see cref="RIS.Connection.MySQL.IRequestEngine"/>, с помощью которого будет выполняться запрос.
        /// </param>
        public SelectFunctionRequest Copy(IRequestEngine engine)
        {
            return new SelectFunctionRequest(engine, this);
        }
        ...
        /// <exception cref="TimeoutException"></exception>
        /// <exception cref="AggregateException"></exception>
        public static string Execute(IRequestEngine engine, string name,
            IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            return Execute(engine, name, Array.Empty<string>(), null, engine.CommandTimeout,
                isolationLevel);
        }
        /// <summary>
        ///     (Синхронно) Вызывает выполнение запроса с заданными параметрами.
        ...
        public static string Execute(IRequestEngine engine, string name,
            TimeSpan timeout, IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            return Execute(engine, name, Array.Empty<string>(), null, timeout,
                isolationLevel);
        }
        /// <summary>
        ...
        /// <exception cref="TimeoutException"></exception>
        /// <exception cref="AggregateException"></exception>
        public static Task<string> ExecuteAsync(IRequestEngine engine, string name,
            IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            return ExecuteAsync(engine, name, Array.Empty<string>(), null, engine.CommandTimeout,
                isolationLevel);
        }
        /// <summary>
        ///     (Асинхронно) Вызывает выполнение запроса с заданными параметрами.
        ...
        public static Task<string> ExecuteAsync(IRequestEngine engine, string name,
            TimeSpan timeout, IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            return ExecuteAsync(engine, name, Array.Empty<string>(), null, timeout,
                isolationLevel);
        }
        ...
        /// <exception cref="TimeoutException"></exception>
        /// <exception cref="AggregateException"></exception>
        public static string Execute(IRequestEngine engine, string name, string table,
            IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            return Execute(engine, name, Array.Empty<string>(), table, engine.CommandTimeout,
                isolationLevel);
        }
        /// <summary>
        ///     (Синхронно) Вызывает выполнение запроса с заданными параметрами.
        ...
        public static string Execute(IRequestEngine engine, string name, string table,
            TimeSpan timeout, IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            return Execute(engine, name, Array.Empty<string>(), table, timeout,
                isolationLevel);
        }
        /// <summary>
        ///     (Асинхронно) Вызывает выполнение запроса с заданными параметрами.
        ...
        /// <exception cref="TimeoutException"></exception>
        /// <exception cref="AggregateException"></exception>
        public static Task<string> ExecuteAsync(IRequestEngine engine, string name, string table,
            IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            return ExecuteAsync(engine, name, Array.Empty<string>(), table, engine.CommandTimeout,
                isolationLevel);
        }
        /// <summary>
        ///     (Асинхронно) Вызывает выполнение запроса с заданными параметрами.
        ...
        public static Task<string> ExecuteAsync(IRequestEngine engine, string name, string table,
            TimeSpan timeout, IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            return ExecuteAsync(engine, name, Array.Empty<string>(), table, timeout,
                isolationLevel);
        }
        ...
        /// <exception cref="TimeoutException"></exception>
        /// <exception cref="AggregateException"></exception>
        public static string Execute(IRequestEngine engine, string name, string[] parametersValues,
            IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            return Execute(engine, name, parametersValues, null, engine.CommandTimeout,
                isolationLevel);
        }
        /// <summary>
        ///     (Синхронно) Вызывает выполнение запроса с заданными параметрами.
        ...
        public static string Execute(IRequestEngine engine, string name, string[] parametersValues,
            TimeSpan timeout, IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            return Execute(engine, name, parametersValues, null, timeout,
                isolationLevel);
        }
        /// <summary>
        ///     (Асинхронно) Вызывает выполнение запроса с заданными параметрами.
        ...
        /// <exception cref="TimeoutException"></exception>
        /// <exception cref="AggregateException"></exception>
        public static Task<string> ExecuteAsync(IRequestEngine engine, string name, string[] parametersValues,
            IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            return ExecuteAsync(engine, name, parametersValues, null, engine.CommandTimeout,
                isolationLevel);
        }
        /// <summary>
        ///     (Асинхронно) Вызывает выполнение запроса с заданными параметрами.
        ...
        public static Task<string> ExecuteAsync(IRequestEngine engine, string name, string[] parametersValues,
            TimeSpan timeout, IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            return ExecuteAsync(engine, name, parametersValues, null, timeout,
                isolationLevel);
        }
        ...
        /// <exception cref="TimeoutException"></exception>
        /// <exception cref="AggregateException"></exception>
        public static string Execute(IRequestEngine engine, string name, string[] parametersValues, string table,
            IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            return Execute(engine, name, parametersValues, table, engine.CommandTimeout,
                isolationLevel);
        }
        /// <summary>
        ///     (Синхронно) Вызывает выполнение запроса с заданными параметрами.
        ...
        public static string Execute(IRequestEngine engine, string name, string[] parametersValues, string table,
            TimeSpan timeout, IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            return new SelectFunctionRequest(engine, name, parametersValues, table,
        ...
        /// <exception cref="TimeoutException"></exception>
        /// <exception cref="AggregateException"></exception>
        public static Task<string> ExecuteAsync(IRequestEngine engine, string name, string[] parametersValues, string table,
            IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            return ExecuteAsync(engine, name, parametersValues, table, engine.CommandTimeout,
                isolationLevel);
        }
        /// <summary>
        ///     (Асинхронно) Вызывает выполнение запроса с заданными параметрами.
