...
{
    public class ChunkedArraySD<T> : IChunkedArray<T>, ICollection, IEnumerable<T>, IEnumerable
    {
        ...
        {
            private readonly ChunkedArraySD<T> _list;
            private long _index;

            public T Current { get; private set; }

            internal Enumerator(ChunkedArraySD<T> list)
            {
                _list = list;
                _index = 0;
                Current = default(T);
        ...
        public ChunkedArraySD(long length, uint chunkSize)
        {
            ChunkSize = chunkSize;
            if (ChunkSize < 1)
            {
