...
{
    public class ChunkedArrayL<T> : IChunkedArray<T>, ICollection, IEnumerable<T>, IEnumerable
    {
        ...
        {
            private readonly ChunkedArrayL<T> _list;
            private long _index;

            public T Current { get; private set; }

            internal Enumerator(ChunkedArrayL<T> list)
            {
                _list = list;
                _index = 0;
                Current = default(T);
        ...
        public ChunkedArrayL(long length)
        {
            ChunkSize = RIS.Environment.GCLOHThresholdSize / RIS.Environment.GetSize<T>();
            if (ChunkSize < 1)
            {
        ...
        public ChunkedArrayL(long length, uint chunkSize)
        {
            ChunkSize = chunkSize;
            if (ChunkSize < 1)
            {
