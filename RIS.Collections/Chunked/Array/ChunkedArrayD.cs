...
{
    public class ChunkedArrayD<T> : IChunkedArray<T>, ICollection, IEnumerable<T>, IEnumerable
    {
        ...
        {
            private readonly ChunkedArrayD<T> _list;
            private long _index;

            public T Current { get; private set; }

            internal Enumerator(ChunkedArrayD<T> list)
            {
                _list = list;
                _index = 0;
                Current = default(T);
        ...
        public ChunkedArrayD(long length)
        {
            ChunkSize = RIS.Environment.GCLOHThresholdSize / RIS.Environment.GetSize<T>();
            if (ChunkSize < 1)
            {
        ...
        public ChunkedArrayD(long length, uint chunkSize)
        {
            ChunkSize = chunkSize;
            if (ChunkSize < 1)
            {
