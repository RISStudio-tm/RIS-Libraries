...
{
    public class ChunkedArrayLog2SD<T> : IChunkedArray<T>, ICollection, IEnumerable<T>, IEnumerable
    {
        ...
        private SortedDictionary<int, T[]> Chunks { get; }
        ...
        {
            private readonly ChunkedArrayLog2SD<T> _list;
            private long _index;

            public T Current { get; private set; }

            internal Enumerator(ChunkedArrayLog2SD<T> list)
            {
                _list = list;
                _index = 0;
                Current = default(T);
            ...
            {
                ChunkedArrayLog2SD<T> list = _list;
            ...
            object IEnumerator.Current
            {
                get
                {
                    if (_index == 0 || _index == _list.Length + 1)
                    {
                        ...
                        Events.OnError(this, new RErrorEventArgs(exception, exception.Message, exception.StackTrace));
                        _list.OnError(new RErrorEventArgs(exception, exception.Message, exception.StackTrace));
                        throw exception;
                    }
        ...
        public ChunkedArrayLog2SD()
            : this(0)
        {

        }
        public ChunkedArrayLog2SD(long length)
            : this(length, Environment.GCLOHThresholdSize / Environment.GetSize<T>())
        {

        }
        public ChunkedArrayLog2SD(long length, uint chunkSize)
        {
            if (chunkSize < 1)
            {
            ...
            Chunks = new SortedDictionary<int, T[]>();
